...
module.exports = kconfig = async (kill, message) => {
    ...
    try {
        ...
        if (isGroupMsg && autoSticker && isMedia && isImage && !isCmd) {
            ...
            await kill.sendImageAsSticker(from, imageBase64, { author: '🎁 https://bit.ly/30t4jJV ☆', pack: 'Robotto', keepScale: true })
        }
		...
		if (isGroupMsg && autoSticker && isMedia && isVideo && !isCmd) {
			...
			await kill.sendMp4AsSticker(from, videoBase64, null, { stickerMetadata: true, pack: 'Robotto', author: '🎁 https://bit.ly/30t4jJV ☆', fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: false, loop: 0 })
		}
        ...
        case 'stiker':
            if (isMedia && isImage) {
                ...
                const url = args[0]
                if (isUrl(url)) {
                    await kill.sendStickerfromUrl(from, url, { method: 'get' }, { author: '🎁 https://bit.ly/30t4jJV ☆', pack: 'Robotto', keepScale: true })
		...
		case 'ttp':
			...
			await kill.sendImageAsSticker(from, ttpst.data.result, { author: '🎁 https://bit.ly/30t4jJV ☆', pack: 'Robotto', keepScale: true })
			break
		...
		case 'about':
			await kill.sendFile(from, './lib/media/img/robotto.png', 'robotto.png', sobre, id)
			break
        ...
        case 'stickernobg':
			if (isMedia) {
                try {
                    ...
                    await kill.sendImageAsSticker(from, `data:${mimetype};base64,${result.base64img}`, { author: '🎁 https://bit.ly/30t4jJV ☆', pack: 'Robotto', keepScale: true })
        ...
        case 'gif':
            if (isMedia && type === 'video' || mimetype === 'image/gif' || isQuotedVideo || isQuotedGif) {
                ...
                try {
                    ...
                    await kill.sendMp4AsSticker(from, gifSticker, null, { stickerMetadata: true, pack: 'Robotto', author: '🎁 https://bit.ly/30t4jJV ☆', fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: false, loop: 0 })
                } catch (err) {
                    console.error(err)
        ...
        case 'makesticker':
            ...
            await kill.sendStickerfromUrl(from, stkfm, { method: 'get' }, { author: '🎁 https://bit.ly/30t4jJV ☆', pack: 'Robotto', keepScale: true })
       ...
       case 'translate':
        ...
        case 'speak':
			...
			try {
				const sprobotto = await axios.get(`http://simsumi.herokuapp.com/api?text=${body.slice(7)}&lang=pt`)
				const a = sprobotto.data.success
        ...
        case 'robotto':
			...
			try {
				const robotto = await axios.get(`http://simsumi.herokuapp.com/api?text=${body.slice(6)}&lang=pt`)
				if (robotto.data.success == '' || robotto.data.success == null || robotto.data.success == 'Limit 50 queries per hour.') {
					await kill.reply(from, resmf, id)
				} else {
					await kill.reply(from, robotto.data.success, id)
					fs.appendFile('./lib/config/reply.txt', `\n${robotto.data.success}`)
				}
			} catch (error) {
				await kill.reply(from, resmf, id)
			}
        ...
        case 'roll':
            ...
            await kill.sendStickerfromUrl(from, 'https://www.random.org/dice/dice' + dice + '.png', { method: 'get' }, { author: '🎁 https://bit.ly/30t4jJV ☆', pack: 'Robotto', keepScale: true })
            break
        ...
        case 'flip':
            ...
            if (limitfp !== undefined && cd - (Date.now() - limitfp) > 0) {
				...
				if (args[0] == 'cara' || args[0] == 'coroa') {
					if (side == 1) {
						await kill.sendStickerfromUrl(from, 'https://i.ibb.co/LJjkVK5/heads.png', { method: 'get' }, { author: '🎁 https://bit.ly/30t4jJV ☆', pack: 'Robotto', keepScale: true })
						if (args[0] == 'cara') {
						...
						await kill.sendStickerfromUrl(from, 'https://i.ibb.co/wNnZ4QD/tails.png', { method: 'get' }, { author: '🎁 https://bit.ly/30t4jJV ☆', pack: 'Robotto', keepScale: true })
						if (args[0] == 'coroa') {
        ...
        case 'mutepv':
            if (isOwner) {
				if (args[0] == 'on') {
					...
					slce.push(pvmt)
					fs.writeFileSync('./lib/config/silence.json', JSON.stringify(slce))
					await kill.reply(from, 'Ele não poderá usar a robotto.', id)
				} else if (args[0] == 'off') {
					if (args.length == 0) return kill.reply(from, 'Você deve definir [on e off] e em seguida o número da pessoa sem - ou +.', id)
					const pvmt = body.slice(11) + '@c.us'
					slce.splice(pvmt, 1)
					fs.writeFileSync('./lib/config/silence.json', JSON.stringify(slce))
					await kill.reply(from, 'Ele poderá usar a robotto novamente.', id)
				} else {
